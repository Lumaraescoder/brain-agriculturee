...
export const RegisterProductor = yup.object().shape({
 ...
 cpf: yup.string().min(6).max(11).required("Cpf cannot be empty"),
 city: yup.string().min(6).max(32).required("City cannot be empty"),
 estate: yup.string().min(6).max(32).required("Estate cannot be empty"),
